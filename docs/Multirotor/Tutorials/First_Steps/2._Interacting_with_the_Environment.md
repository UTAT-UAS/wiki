...
Note that the PX4 guideline uses `make px4_sitl gz_x500`, which internally start both gazebo client and server in the VM. To set up server and client separately, use the following command on your VM to start the server.
...
GZ_IP=<IP_OF_THE_HOST> GZ_PARTITION=1 GZ_RELAY=<IP_OF_THE_VM> GZ_VERBOSE=1 GZ_TRANSPORT_RCVHWM=100000 GZ_TRANSPORT_SNDHWM=100000 GZ_TRANSPORT_TOPIC_STATISTICS=0 gz sim -g -v4 x500/model.sdf
```
