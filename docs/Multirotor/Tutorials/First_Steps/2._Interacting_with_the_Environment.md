...
If you are coming from Windows `apt` is similar to `winget`, `scoop`, or `choco`. On macOS the equivalent is `brew`.
...
> We avoid X11 forwarding for the simulation in this setup because configuring GPU acceleration over X11 is complex.

Since Gazebo server and client can run separately, we can start gazebo as a server on the VM, and as the client on the Host, allowing Gazebo to render using the host's GPU.
...
You might encounter issues with QGroundControl not receiving MAVLink traffic from PX4 in the VM. If this is the case, verify the local UDP port of MAVLink using the following command in the PX4 console:

```
mavlink status
```

Next add a remote UDP link in your QGroundControl under Settings > Comm Links, with the network address MAVLink is on (i.e. the IP address of the VM combined with MAVLink UDP port).
